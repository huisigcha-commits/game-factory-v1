...
const card = (game) => { const cover = coverAssetBySlug[game.slug]; return `<article class="game-card"><div class="game-art art-${game.category}">${cover ? `<img src="/assets/covers/${cover}" alt="${game.title} 게임 장면" loading="lazy" />` : ''}<span aria-hidden="true">${game.id}</span><small>${game.category}</small></div><div class="game-card-body"><p class="eyebrow">${game.category}</p><h3>${game.title}</h3><p>${game.shortDescription}</p><div class="card-actions"><a class="play-link" href="/games/${game.slug}/">Play <span aria-hidden="true">→</span></a></div></div></article>`; };
