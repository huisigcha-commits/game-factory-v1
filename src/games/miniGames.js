...
export function mountGame({ container, sdk, game, onScore }) {
  ...
  const renderTiming = () => { let position = 0, direction = 1; shell(`<div class="mini-track"><span class="mini-marker"></span><b class="mini-target"></b></div><button class="mini-action">DROP</button><p class="mini-game-note">초록 구간을 맞히면 점수가 오릅니다.</p>`); const marker = container.querySelector('.mini-marker'); timer = window.setInterval(() => { position += direction * (2 + score*.15); if(position>96||position<0) direction *= -1; marker.style.left = `${position}%`; }, 22); container.querySelector('.mini-action').onclick = () => { if(!running)return; sdk.firstAction(); if (position >= 42 && position <= 58) { updateScore(score+1); } else gameOver(); }; };
