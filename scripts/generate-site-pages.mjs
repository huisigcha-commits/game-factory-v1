...
for (const [path, title, attributes] of listingPages) {
  ...
  writeFileSync(new URL(`${path}/index.html`, root), shell({ title, attributes, script: 'listPage', depth: true }));
}
for (const category of categories) {
  const directory = new URL(`category/${category.id}/`, root);
  mkdirSync(directory, { recursive: true });
  writeFileSync(new URL('index.html', directory), shell({ title: category.label, attributes: `data-view="category" data-category="${category.id}"`, script: 'listPage', depth: true }));
}
for (const [path, title] of [['about', 'About'], ['contact', 'Contact'], ['privacy', 'Privacy'], ['terms', 'Terms']]) {
  mkdirSync(new URL(`${path}/`, root), { recursive: true });
  writeFileSync(new URL('index.html', new URL(`${path}/`, root)), shell({ title, attributes: `data-page="${path}"`, script: 'staticPage', depth: true }));
}
